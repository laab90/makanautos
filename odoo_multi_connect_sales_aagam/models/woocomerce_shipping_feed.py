...
from odoo.addons.odoo_multi_connect_sales_aagam.tools import extract_list as EL
import logging
...
class ShippingFeed(models.Model):
    ...
    def create_shipping_mapping(self, channel_id, carrier_id, vals):
        vals = dict(
            ...
            store_selection=channel_id.channel,
        ...
        return self.env['shipping.mapping'].create(vals)
    ...
    def import_item(self):
        ...
        map_domain = channel_domain + \
            [('shipping_service_id', '=', vals.get('store_id'))]
        match = self.env['shipping.mapping'].search(
            map_domain, limit=1)
        ...
        if state == 'done':
            if not match:
                ...
                if res:
                    message += 'Shipping carriers has been updated Successfully'
