...
class MultiChannelSkeleton(models.Model):
    ...
    def CreatePaymentMethod(self, ChannelID, PaymentMethod):
        ...
        if PaymentMethod:
            journal_id = 0
            MapObj = self.env['woo.comm.journal.mapping']
            exists = MapObj.search(
                [('wc_journal_name', '=', PaymentMethod)])
            if not exists:
                ...
                if journal_obj:
                    MapVals = {
                        ...
                        'ref_odoo_journal_id': journal_id,
                        'channel_id': ChannelID.id,
                        'store_selection': ChannelID.channel}
                    MapObj.create(MapVals)
            else:
                journal_id = exists[0].ref_odoo_journal_id
        return {
            'journal_id': journal_id,
        ...
        if counter <= 1:
            try:
                ...
                if len(open_inv) and (payment_data['invoice_state'] == 'paid'):
                    ctx = {'default_invoice_ids': [[4, invoice_id, None]], 'active_model': 'account.move', 'journal_type': 'sale',
...
class ChannelOrderStates(models.Model):
    _name = "woo.comm.sale.order.state"
    ...
    channel_id = fields.Many2one('woo.comm.channel.sale', 'Channel')
    default_order_state = fields.Boolean('Default State', help="Default state if store state does not match the odoo state.")
    odoo_create_invoice = fields.Boolean('Create Invoice', help="Create invoice in odoo.")
    odoo_set_invoice_state = fields.Selection([ ('open', 'Open'), ('paid', 'Paid')],
        string='Set Invoice State',
        help="The field is used to retrieve orders that are in a specific state.")
    odoo_ship_order = fields.Boolean("Create Shippment")
    _sql_constraints = [
