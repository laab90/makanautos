...
class WooCommerceFeed(models.Model):
    _name = "woo.comm.feed"
    ...
    state = fields.Selection(
        ...
        default='draft',
        copy=False)
    channel_id = fields.Many2one('woo.comm.channel.sale', string='Instance', domain=[('state', '=', 'validate')])
    channel = fields.Selection(related='channel_id.channel', string="Channel")
    ...
    def open_mapping_view(self):
        ...
        duplicity_config=self.env['res.config.settings'].get_values()
        domain = [
            ('channel_id', '=', channel_id.id),
        ]
        if res_model in ['product.mapping','template.mapping'] and duplicity_config.get('is_duplicate_avoid'):
...
class CategoryFeed(models.Model):
    ...
    is_leaf_category = fields.Boolean('Leaf Category')
    ...
    def import_category(self,channel_id):
        ...
        if match:
            if state == 'done':
                update_id = match
                try:
                    match.product_category_id.write(vals)
            ...
            if state == 'done':
                try:
                    ...
                    create_id = channel_id.create_category_mapping(
                        erp_id, store_id, self.is_leaf_category
                    )
    ...
    def import_items(self):
        ...
        for record in self:
            ...
            if mapping_id:
                ...
                sync_vals['ecomstore_refrence'] = mapping_id.wc_product_categ_id
                sync_vals['odoo_id'] = mapping_id.categ_id
...
class ProductVaraintFeed(models.Model):
    ...
    name_value = fields.Text('Name Value list',
    ...
    extra_categ_ids = fields.Text('Extra Category ID(s)',
    ...
    feed_templ_id = fields.Many2one('product.feed',
        string='Template ID',
        ondelete='cascade',
    )
    product_id_type = fields.Char('Product ID Type')
...
class ProductFeed(models.Model):
    ...
    feed_variants = fields.One2many('product.variant.feed','feed_templ_id', string = 'Varinats')
    store_selection = fields.Selection(
        selection='eComStoreUsed',
        string="Channel",
    )

    multi_image_url_ids = fields.One2many('product.feed.multi.image.url','product_feed_id')
        ...
        for variant in variant_objs:
            ...
            if not exists:
                ...
                if variant.list_price:
                    price =parse_float(variant.list_price)
                    self.change_product_price(
                        product_id = exists.product_name,
                        price = price,
                        channel_id = channel_id
                    )
                if context.get('ax_qty_update',True) and  variant.qty_available and eval(variant.qty_available) > 0:
                    res = self.change_product_qty(
    ...
    def get_variant_extra_values(self, template_id, variant,channel_id):
        ...
        if variant.image_1920:
            vals.update({'image_1920': variant.image_1920})
        else:
        ...
        if dimensions_unit:
            vals['measure_id'] = channel_id.get_uom_id(
                name=dimensions_unit).id
        if not variant.product_id_type:
            vals['product_id_type'] = 'upc_no'
        else:
            vals['product_id_type'] = variant.product_id_type
        if variant.description_sale:
            vals['description_sale'] = variant.description_sale
    ...
    def _create_product_line(self, variant, template_id, store_id, location_id,channel_id):
        ...
        if not exists:
            ...
            # vals.update({'product_template_attribute_value_ids':  [(6, 0, [51])]})
            ...
            if not product_exists_odoo:
                product_id = prod_env.create({
                'name': self.name,
                'lst_price':14,
                
                })
                product_product_id = self.env['product.template'].search([('id', '=', template_id.id)])
    ...
    @api.model
    def change_product_price(self, product_id, price, channel_id):
        product_product_id = self.env['product.product'].search([('id','=',product_id.id)])
        product_product_id.update({'lst_price':price})
    ...
    @api.model
    def change_product_qty(self, product_id, qty_available, location_id, template_id):
    ...
    def import_product(self,channel_id):
        ...
        if dimensions_unit:
            vals['measure_id'] = channel_id.get_uom_id(
                name=dimensions_unit).id
        if not vals.pop('product_id_type'):
            vals['product_id_type'] = 'upc_no'
        ...
        # multi image
        multi_image_url_ids = vals.pop('multi_image_url_ids')
        multi_image_url_rec = self.env['product.feed.multi.image.url'].search([('id','in',multi_image_url_ids)])
        
        location_id = channel_id.location_id
        if not vals.get('image_1920') and image_url and (image_url not in ['false','False',False]):
            image_res = channel_id.read_website_image_url(image_url)
            product_img_list = []
            for img in multi_image_url_rec:
                multi_image_res = channel_id.read_website_image_url(img.multi_image_url)
                if multi_image_res:
                    product_img_list.append((0,0, {
                        'name':img.multi_image_url,
                        'image_1920':multi_image_res,
                    }))
            if product_img_list:
                vals['product_template_image_ids'] = product_img_list
        ...
        if state == 'done':
            if match:
                ...
                extra_categ = self.env['extra.product.categories'].search([('instance_id','=',channel_id.id), ('product_id','=', template_id.id)])
                if extra_categ_ids:
                ...
                if len(variant_lines):
                    ...
                    for variant_id in template_id.product_variant_ids:
                        ...
                        # match_product = channel_id.match_product_mappings(
                        #     store_id, variant_id, default_code=variant_vals.get('default_code'),
                        #     barcode=variant_vals.get('barcode'))
                        # if not match_product:
                        #     channel_id.create_product_mapping(
                        #         template_id, variant_id, store_id, variant_id,
                        #         {'default_code': variant_vals.get('default_code'),
                        #          'barcode': variant_vals.get('barcode')})
                update_id = match
                ...
                try:
                    ...
                    if not template_exists_odoo:
                        _logger.info("template id in product.template==========if= ")
                        if variant_lines:
                            context['create_product_product']=1
                        vals['image_1920'] = vals.pop('image_1920')
                        extra_categ_ids = vals.pop('extra_categ_ids')
                        if extra_categ_ids:
                            res = self.get_extra_categ_ids(extra_categ_ids,channel_id)
                            message += res.get('message', '')
                            ...
                            if categ_ids:
                                ...
                                extra_categ = self.env['extra.product.categories'].with_context(context).create(data)
                                vals['extra_categ_ids'] = [(6, 0, [extra_categ.id])]
                        ...
                        _logger.info("template id in product.template==========else= ")
                        template_id = template_exists_odoo
                        extra_categ_ids = vals.pop('extra_categ_ids')
                        if extra_categ_ids:
                            res = self.get_extra_categ_ids(extra_categ_ids,channel_id)
                            ...
                            if categ_ids:
                                ...
                                template_id.extra_categ_ids = [(0, 0, data)]
                            else:
                                state = 'error'
                                template_id = None
                    ...
                    if len(variant_lines) and template_id:
                        ...
                        for variant_id in template_id.product_variant_ids:
                            variant_vals = variant_id.read(['default_code','barcode'])[0]
                            abcd = self.change_product_price(
                                product_id =variant_id,
                                price = list_price,
                                channel_id = channel_id
                            )
                            if qty_available and eval(qty_available) > 0:
                                self.change_product_qty(
                                    variant_id, qty_available, location_id, template_id)
                            match = channel_id.match_product_mappings(
                                store_id, 'No Variants', default_code=variant_vals.get('default_code'), barcode=variant_vals.get('barcode'))
                            if not match:
                                channel_id.create_product_mapping(
...
class ProductFeedMultiImageUrl(models.Model):
    _name = "product.feed.multi.image.url"

    product_feed_id = fields.Many2one('product.feed')    
    multi_image_url = fields.Char()
