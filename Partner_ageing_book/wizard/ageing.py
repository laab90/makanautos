    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data ):
        ...
        if filters.get('date_from'):
            sheet.merge_range('A4:B4',
                              'A la fecha: ' + filters.get('date_from'),
                              date_head)
        sheet.merge_range('C4:E4',
                          'Tipo de cuenta: ' + filters.get('result_selection'),
                          date_head)
        sheet.merge_range('A5:B5',
                          'Movimientos objetivo: ' + filters.get('target_move'),
                          date_head)
        sheet.merge_range('D5:F5', '  Socios: ' + ', '.join(
            [lt or '' for lt in
             filters['partners']]), date_head)
        sheet.merge_range('G5:H5', ' Tipo de socio: ' + ', '.join(
            [lt or '' for lt in
             filters['partner_tags']]),
                          date_head)

        sheet.merge_range('A7:C7', 'Socio', heading)
        sheet.write('D7', 'Total', heading)
        sheet.write('E7', 'No vencido', heading)
        ...
        for rec_data in report_data_main[0]:
            ...
            row += 1
            sheet.write(row, col, 'Etiqueta de entrada', sub_heading)
            sheet.write(row, col + 1, 'Fecha de vencimiento', sub_heading)
            sheet.write(row, col + 2, 'Diario', sub_heading)
            sheet.write(row, col + 3, 'Cuenta', sub_heading)
            sheet.write(row, col + 4, 'No vencido', sub_heading)
